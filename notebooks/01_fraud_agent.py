...
# MAGIC
# MAGIC **Domain:** Healthcare insurance claims (Upcoding, Phantom Billing, Provider Networks)
# MAGIC
# MAGIC **Authentication:** Uses WorkspaceClient (same as dashboard) for realistic testing
# MAGIC
# MAGIC **Branch:** `main` (production-ready demo)
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC 4. ✅ Tests agent with different claim complexities (simple → suspicious → complex)
# MAGIC 5. ✅ Compares Sequential vs Agent approaches (cost optimization)
# MAGIC
# MAGIC **Goal:** Demonstrate how the fraud detection agent works under the hood
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC - `VectorSearchClient` - For semantic search in fraud knowledge base
...
# MAGIC %md
# MAGIC ## ⚙️ Configuration from config.yaml
# MAGIC
# MAGIC **Centralized Configuration Strategy:**
# MAGIC - Uses `shared/config.py` module (same as dashboard and all setup notebooks)
# MAGIC - Reads from `config.yaml` for single source of truth
# MAGIC - Auto-detects environment (dev/staging/prod) from DAB widget or env variable
# MAGIC - All settings (catalog, schema, warehouse, vector index, LLM) come from config
# MAGIC
# MAGIC **Benefits:**
# MAGIC - ✅ No hardcoded values - change `config.yaml` once, works everywhere
# MAGIC - ✅ Environment-specific settings (dev vs prod)
# MAGIC - ✅ Consistent with dashboard and setup notebooks
# MAGIC - ✅ Easy to maintain and update
# MAGIC
# MAGIC **LLM Endpoint:**
# MAGIC - Production uses Claude Sonnet 4 for reliable function calling
# MAGIC - Best JSON format compliance for tool calls
# MAGIC - Configured per environment in `config.yaml`
...
# MAGIC %md
# MAGIC ## 💬 Tool 3: UC Function - Fraud Explanation
...
# MAGIC %md
# MAGIC ## 🔍 Tool 4: Vector Search Wrapper
...
# MAGIC %md
# MAGIC ## 🤖 Tool 5: Genie Conversation API Wrapper
...
# MAGIC - LangChain v1.0: Use `langchain.claims_analysis.create_agent()` with state_schema
# MAGIC
...
# MAGIC %md
# MAGIC ---
# MAGIC # Part 4: Test Agent with Different Claims
# MAGIC
# MAGIC See how the agent adapts to different claim complexities
# MAGIC
...
# MAGIC
# MAGIC **Key Points:**
# MAGIC - Simple claims use 2-3 tools (fast, cheap)
# MAGIC - Complex claims use all 4 tools (thorough, accurate)
# MAGIC - Agent decides adaptively based on claim content
...
# MAGIC %md
# MAGIC ## 🧪 Test 1: Simple Legitimate Claim (Expected: 2 tools)
...
# MAGIC %md
# MAGIC ## 🧪 Test 2: High-Risk Fraud Claim (Expected: 4 tools)
...
# MAGIC %md
# MAGIC ## 🧪 Test 3: Suspicious Fraud Pattern (Expected: 3 tools)
...
# MAGIC
# MAGIC - ✅ **Tool 1 Working:** UC Function (fraud_classify) with WorkspaceClient
# MAGIC - ✅ **Tool 2 Working:** UC Function (fraud_extract_indicators) with WorkspaceClient
...
# MAGIC - ✅ **Agent Makes Decisions:** Tested with different claim complexities
...
# MAGIC 1. **Agent adapts to claim complexity** - uses fewer tools for simple questions
...
# MAGIC - **Sequential:** Predictable, uniform claims; need consistency
# MAGIC - **Agent:** Varied complexity claims; want cost optimization
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 🚀 Next Steps
